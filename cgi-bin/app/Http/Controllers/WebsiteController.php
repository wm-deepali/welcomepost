...
// 								->sum('amount');
    ...
    public function payouts(Request $request)
    {
		...
		$data['commission'] = CustomerCommission::where('parent_id', session('id'))
			...
			->select(
				'id',
				'status',
				'created_at','reason','image','payment_method',
	...
	//	$data['allpost']			= Adposting::where('ad_expiry' , '=', NULL)->where('status','1')->where('delete_status','0')->where('category_id',$id)->get();
		$data['allpost']			= Adposting::where('status','1')->where('delete_status','0')->where('category_id',$id)->get();
		$data['subscription']  		= Subscription::where('delete_status','0')->where('status','0')->orderby('offered_price','DESC')->get();
		$data['Pages']				= Pages::where('delete_status','0')->get();
		$data['city']				= City::where('delete_status','0')->orderby('name','asc')->get();
		$userIp 					= $request->ip();
    ...
    public function subcategory_ads(Request $request,$id)
    {
		...
		$data['allpost']			= Adposting::where('delete_status','0')->where('status','1')->where('sub_category_id',$id)->get();
		$data['Pages']				= Pages::where('delete_status','0')->get();
		$data['city']				= City::where('delete_status','0')->orderby('name','asc')->get();
		$userIp 					= $request->ip();
		$data['locationinfo'] 		= \Location::get($userIp);	
    ...
    public function city_ads(Request $request)
    {
		...
		$data['allcategories']		= Categories::where('delete_status','0')->orderby('name','ASC')->get();
		$data['allsubcategories']	= Subcategories::where('delete_status','0')->where('id',$id)->get();
		$query = Adposting::where('status', '1')->where('delete_status', '0');
    ...
    public function category_location_ads(Request $request)
    {
		...
		$data['allpost']			= Adposting::where('status','1')->where('delete_status','0')->where('category_id',$category)->where('city',$city)->get();
		$data['Pages']				= Pages::where('delete_status','0')->get();
		$data['city']				= City::where('delete_status','0')->orderby('name','asc')->get();
		$userIp 					= $request->ip();
		$data['locationinfo'] 		= \Location::get($userIp);
    ...
    public function ads_details(Request $request,$id)
    {
		...
		$data['latestpost']			=Adposting::where('delete_status','0')->where('status','1')->where('category_id',$ctgid)->where('active_status', 1)->where('id', '!=', $id)->orderby('id','desc')->limit(3)->get();	
		$user_id = session()->get('id');
    ...
    public function all_product(Request $request)
    {
		...
		$data['jobads']				= Jobforms::where('delete_status','0')->where('status','1')->orderby('id','desc')->get();
		$data['homecategories']		= Categories::where('delete_status','0')->limit(8)->get();
		$data['allpost']			= Adposting::where('status','1')->where('delete_status','0')->get();    
    ...
    public function deactivate_account(Request $request)
    {
    	if (session()->has('id')) {
            ...
            if ($user && Hash::check($old_pswd, $user->password)) {
                ...
                $user->deleted_at = now();
                $user->save();
                $adsposting = Adposting::where('user_id',$user_id)->get();
    ...
    public function my_ads(Request $request)
    {
    	if (session()->has('id')) 
    	{
			...
			$data['locationinfo']   = \Location::get($userIp);
    		$data['page'] 	= 'My Ads';
    		$data['my_ads'] = Adposting::where('user_id',$user_id)->where('delete_status',0)->orderby('id','desc')->get();
    	    $data['history'] 	= DB::table('subscription_orders')->where('user_id',$user_id)->where('delete_status',0)->orderby('id','desc')->get();	
	...
	public function post_job_form(Request $request)
    {
		...
		if($category_subscription_exists)
		{
            ...
            if($ads_postings_count>=$remainingAdsTotal){
			    ...
			    Session::put('UserProfile', ['image'=>$imageSession,'ads_validity'=>$ads_validity,'ad_id'=>$rand = mt_rand(1500, 5000),'category_id' => $request->category_id,'user_id'=>$request->user_id,'sub_category_id'=>$request->subcatid,'formtype'=>$request->formtype,'fullname'=>$request->fullname,
    		     'email'=>$request->email,'mobile'=>$request->mobile,'location'=>$request->ip(),'city'=>$request->city,'price'=>$request->price,'ad_title'=>$request->ad_title,
    			...
    			'fullname'=>$request->fullname,'email'=>$request->email,'mobile'=>$request->mobile,'location'=>$request->ip(),'state'=>$request->state,'state_name'=>$request->state_name,'city'=>$request->city,
			...
			if($status == '1')
			{
			    ...
			    Session::put('UserProfile', ['image'=>$imageSession,'ads_validity'=>$ads_validity,'ad_id'=>$rand = mt_rand(1500, 5000),'category_id' => $request->category_id,'user_id'=>$request->user_id,'sub_category_id'=>$request->subcatid,'formtype'=>$request->formtype,'fullname'=>$request->fullname,
    		     'email'=>$request->email,'mobile'=>$request->mobile,'location'=>$request->ip(),'city'=>$request->city,'salary_from'=>$request->salary_from,'salary_to'=>$request->salary_to,'ad_title'=>$request->ad_title,
    		     'ad_type'=>'Free','active_status'=>'0','ad_view_count'=>'0','description'=>$request->description,'delete_status'=>'0','status'=>'0']);
    			Session::put('ImageAd',['image1'=>$imageSession??null,'image2'=>$imageSession2??null,'image3'=>$imageSession3??null,'image4'=>$imageSession4??null,]);
    			Session::put('JobDetail',['user_id'=>$request->user_id,'category_id'=>$request->category_id,'sub_category_id'=>$request->subcatid,'formtype'=>$request->formtype,
    			'fullname'=>$request->fullname,'email'=>$request->email,'mobile'=>$request->mobile,'location'=>$request->ip(),'state'=>$request->state,'state_name'=>$request->state_name,'city'=>$request->city,
    			'city_name'=>$request->city_name,'neibourhood'=>$request->neibourhood,'salary_period'=>$request->salary_period,'ad_title'=>$request->ad_title,'ad_type'=>'Free','description'=>$request->description,
    			'position_type'=>$request->position_type,'salary_from'=>$request->salary_from,'salary_to'=>$request->salary_to,'delete_status'=>'0','status'=>'0']);
	...
	//	{
		 ...
		 //	if($no_of_ads == $ads_postings_count  && $category_subscription_exists != 1)
		 	if($category_subscription_exists != 1)
		 	{
    			...
    			Session::put('MobileDetail',['user_id'=>$request->user_id,'category_id'=>$request->category_id,'sub_category_id'=>$request->subcatid,'formtype'=>$request->formtype,
    			'fullname'=>$request->fullname,'email'=>$request->email,'mobile'=>$request->mobile,'location'=>$request->ip(),'state'=>$request->state,'state_name'=>$request->state_name,'city'=>$request->city,
    			'city_name'=>$request->city_name,'neibourhood'=>$request->neibourhood,'brand'=>$request->brands,'ad_title'=>$request->ad_title,'ad_type'=>'Free','description'=>$request->description,
    			'delete_status'=>'0','status'=>'0']);
         ...
         $pending_ads_count = DB::table('ads_postings')
        	...
        	$category_id_exists = DB::table('subscriptions_free_trials')->where('category_id',$category_id)->exists();
        		...
        		if($category_subscription_exists != 1)
        		{
        			...
        			Session::put('VehicleDetail',['user_id'=>$request->user_id,'category_id'=>$request->category_id,'sub_category_id'=>$request->subcatid,'formtype'=>$request->formtype,
        			'fullname'=>$request->fullname,'email'=>$request->email,'mobile'=>$request->mobile,'location'=>$request->ip(),'state'=>$request->state,'state_name'=>$request->state_name,'city'=>$request->city,
        			'city_name'=>$request->city_name,'neibourhood'=>$request->neibourhood,'price'=>$request->price,'ad_title'=>$request->ad_title,'ad_type'=>'Free','description'=>$request->description,
        			'brand'=>$request->brands,'vehicle_type'=>$request->vehicle_type,'fuel_type'=>$request->fuel_type,'transmission'=>$request->transmission,'year'=>$request->year,'km'=>$request->km,
        			'delete_status'=>'0','status'=>'0']);
    	...
    	//	if($no_of_ads == $ads_postings_count  && $category_subscription_exists != 1)
    		if($category_subscription_exists != 1)
    		{
    		   if($request->hasFile('file'))
    			...
    			Session::put('PropertyDetail',['user_id'=>$request->user_id,'category_id'=>$request->category_id,'sub_category_id'=>$request->subcatid,'formtype'=>$request->formtype,
    			'fullname'=>$request->fullname,'email'=>$request->email,'mobile'=>$request->mobile,'location'=>$request->ip(),'state'=>$request->state,'state_name'=>$request->state_name,'city'=>$request->city,
    			'city_name'=>$request->city_name,'neibourhood'=>$request->neibourhood,'price'=>$request->price,'ad_title'=>$request->ad_title,'ad_type'=>'Free','description'=>$request->description,
    			'property_type'=>$request->property_type,'bedroom'=>$request->bedroom,'bathroom'=>$request->bathroom,'furnishing_status'=>$request->furnishing_status,'construction_status'=>$request->construction_status,
    			'residence'=>$request->residence_status,'listed_by'=>$request->listed_by,'plot_type'=>$request->plot_type,'price_mention'=>$request->price_mention,'builtup_area'=>$request->builtup_area,
    			'carpet_area'=>$request->carpet_area,'maintenance'=>$request->maintenance,'total_floor'=>$request->total_floor,'floor_no'=>$request->floor_no,'car_parking'=>$request->car_parking,
    			'facing'=>$request->facing,'project_name'=>$request->project_name,'delete_status'=>'0','status'=>'0']);
...
// 			$userprofile->auto_join_member 			= $managecommission->auto_join_member;
    ...
    public function top_search(Request $request)
    {
        ...
        ->where('ad_expiry' , '!=', NULL)->where('delete_status','0')->get();
